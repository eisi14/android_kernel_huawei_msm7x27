...
static struct platform_device lcdc_ili9486_panel_device = 
{
    .name   = "lcdc_ili9486_hvga",
    .id     = 0,
    .dev    = {
        .platform_data = &lcdc_gordon_panel_data,
    }
...
int touch_power(int on)
{
      	...
      	if (on)
	{
	     ...
	     return rc_synt;
          ...
          /* set voltage as 2800 in c8651 */
          rc_synt=vreg_set_level(vreg_synt, 2800);
          if(rc_synt)
          {
...
static struct msm_camera_sensor_info msm_camera_sensor_mt9p017_data = {
	...
	.flash_type		= MSM_CAMERA_FLASH_LED, //DTS2011051803199 h00144661 add
...
static void __init virtualkeys_init(void)
{
    ...
    else if( machine_is_msm7x27_m865() || machine_is_msm7x27_u8650() || machine_is_msm7x27_c8650() )
    {
        buf_vkey_size = sprintf(buf_virtualkey,
    ...
    {
        buf_vkey_size = sprintf(buf_virtualkey,
        			__stringify(EV_KEY) ":" __stringify(KEY_SEND)  ":35:520:70:60"
        		   ":" __stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":118:520:70:60"
        		   ":" __stringify(EV_KEY) ":" __stringify(KEY_BACK)   ":205:520:70:60"
        		   ":" __stringify(EV_KEY) ":" __stringify(KEY_END) ":285:520:70:60"
        		   "\n");
    }
    else
    {
